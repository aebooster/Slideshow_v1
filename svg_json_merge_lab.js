...
<body>
<div class="hdr"><div class="hdr-b"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--acc)" stroke-width="2"><polygon points="12 2 22 8.5 22 15.5 12 22 2 15.5 2 8.5"/><line x1="12" y1="22" x2="12" y2="15.5"/><polyline points="22 8.5 12 15.5 2 8.5"/></svg>HUIGMA</div><div class="hdr-a"><a href="/" class="btn" title="На главную">🏠</a><button class="btn" id="saveBtn" onclick="saveTemplate()" title="Ctrl+S">💾 Save</button><button class="thm" onclick="toggleTheme()" id="thmB">☀</button><button class="btn" onclick="$('fI').click()">Open SVG</button><button class="btn" onclick="$('packI').click()">Import SVG+Layout</button><button class="btn" onclick="openM('ljM')">Paste Layout JSON</button><button class="btn" onclick="openM('fM')">Fonts <span class="bdg" id="fBdg">0</span></button><button class="btn" onclick="openM('aM')">API <span class="bdg" id="dBdg">0</span></button><input type="file" id="fI" accept=".svg" style="display:none" onchange="handleFile(this.files[0])"><input type="file" id="packI" accept=".zip,.json" style="display:none" onchange="handlePackFile(this.files[0])"></div></div>
<div class="ws">
...
<div class="tb bot"><button class="btn" onclick="zoomBy(-.15)">−</button><span class="zl" id="zL">100%</span><button class="btn" onclick="zoomBy(.15)">+</button><div class="tsep"></div><button class="btn" onclick="fitV()">Fit</button></div>
</div>
...
<!-- LAYOUT JSON MODAL -->
<div class="mo" id="ljM" onclick="if(event.target===this)closeM('ljM')"><div class="md" style="width:780px"><div class="mh"><span class="mt">🧩 Layout JSON</span><button class="mx" onclick="closeM('ljM')">&times;</button></div><div class="mb"><div class="ms"><p style="font-size:11px;color:var(--t2);line-height:1.5">Вставьте JSON целиком. Поддерживается: <code style="background:var(--bgH);padding:1px 5px;border-radius:3px;font-size:10px">layoutJson/layout/document/root/nodes</code>.</p><textarea id="ljTA" class="gfi" style="min-height:260px;resize:vertical;width:100%;font-family:'JetBrains Mono',monospace" placeholder='{"layoutJson": {...}}'></textarea><div style="display:flex;gap:6px;margin-top:10px"><button class="btn btn-p" onclick="applyLayoutJSONText()">Применить JSON</button><button class="btn" onclick="analyzeJsonSvgIds()">Сверить ID</button><button class="btn" onclick="$('ljTA').value=''">Очистить</button><button class="btn" onclick="clearLayoutLogs()">Очистить лог</button></div><div id="ljST" style="margin-top:8px;font-size:10px;color:var(--t3)"></div><div id="ljMAP" style="margin-top:8px;max-height:200px;overflow:auto;background:var(--inp);border:1px solid var(--brd);border-radius:6px;padding:8px;font-size:10px;color:var(--t2)"></div><pre id="ljLOG" style="margin-top:8px;max-height:150px;overflow:auto;background:var(--inp);border:1px solid var(--brd);border-radius:6px;padding:8px;font-size:10px;color:var(--t2);white-space:pre-wrap"></pre></div></div></div></div>
...
const LAY=new Map();
const GN=new Map(); // nid → custom group name // nid → {on:bool, dir:'h'|'v', gap:N, pt:N, pr:N, pb:N, pl:N, align:'start'|'center'|'end', wrap:bool} // nid → color index (0=none)
...
const svgNormMap = new Map();
const svgNameMap = new Map();
const svgTailMap = new Map();
const LM = new Map();
const layoutToSvg = (window.layoutToSvg instanceof Map)?window.layoutToSvg:new Map();
window.layoutToSvg = layoutToSvg;
const layoutBase = new WeakMap();
const colorCanvasCtx = document.createElement('canvas').getContext('2d');
let rootNode = null;
function onSvgClick(e){
  const hit = e.target.closest?.('[data-nid]');
  if(!hit||!svg||!svg.contains(hit))return;
  e.stopPropagation();
  selN(hit.dataset.nid,e);
}
...
function node(id,type){
  return {
    ...
    hasAbs:false,
    orig:{x:0,y:0},
    layout:{mode:'NONE',gap:0,padding:{t:0,r:0,b:0,l:0}},
    size:{w:0,h:0},
    pos:{x:0,y:0},
    text:{content:'',maxWidth:null}
  }
}

function build(n,parent=null){
  const x = node(n.id,n.type);
  LM.set(x.id,x);
  const ly = n.layout||n;
  ...
  if(ly.layoutMode){
    x.layout.mode = ly.layoutMode==='VERTICAL'?'VERTICAL':ly.layoutMode==='HORIZONTAL'?'HORIZONTAL':'NONE';
    x.layout.gap = ly.itemSpacing||0;
    x.layout.padding = {
      t:ly.paddingTop||0,
      r:ly.paddingRight||0,
      b:ly.paddingBottom||0,
      l:ly.paddingLeft||0
    }
  }

  if(n.type==='TEXT'){
    x.text.content = n.text?.characters||n.characters||'';
    if(n.absoluteBoundingBox){
      x.text.maxWidth = n.absoluteBoundingBox.width;
    }else if(n.bounds?.width){
      x.text.maxWidth = n.bounds.width;
    }
  }

  if(n.absoluteBoundingBox){
    x.abs.x = n.absoluteBoundingBox.x||0;
    x.abs.y = n.absoluteBoundingBox.y||0;
    x.size.w = n.absoluteBoundingBox.width;
    x.size.h = n.absoluteBoundingBox.height;
    x.hasAbs = true;
    x.orig.x = x.abs.x;
    x.orig.y = x.abs.y;
  }
  ...
  if(!x.hasAbs){
    const el=layoutToSvg.get(x.id)||svgMap.get(x.id);
    if(el){
      const b=getElementMetrics(el);
      x.orig.x=b.x||0;
      x.orig.y=b.y||0;
      if(!x.size.w&&!x.size.h){x.size.w=b.width||0;x.size.h=b.height||0;}
    }
  }

  if(n.children){
    n.children.forEach(c=>build(c,x));
  }
...
function mapSVG(){
  ...
  svg.querySelectorAll('*').forEach(el=>{
    const fid=el.getAttribute('data-figma-id');
    const id=el.getAttribute('id');
    const rawKeys=[fid,id].filter(Boolean);
    const fixedKeys=rawKeys.map(fixMojibake).filter(Boolean);
    const allKeys=[...new Set([...rawKeys,...fixedKeys])];
    allKeys.forEach(k=>{if(!svgMap.has(k))svgMap.set(k,el);});
    allKeys.forEach(k=>{const nk=normK(k);if(nk&&!svgNormMap.has(nk))svgNormMap.set(nk,el);});
    const rawName=el.getAttribute('data-name')||id||'';
    const nameKeys=[rawName,fixMojibake(rawName)].filter(Boolean).map(normK).filter(Boolean);
    nameKeys.forEach(nName=>{
      if(!svgNameMap.has(nName))svgNameMap.set(nName,[]);
      svgNameMap.get(nName).push(el);
    });
    allKeys.forEach(k=>{
      const tail=tailK(k);
      if(tail){
        if(!svgTailMap.has(tail))svgTailMap.set(tail,[]);
        svgTailMap.get(tail).push(el);
      }
    });
  });
}
...
function fixMojibake(s){
  try{
    ...
    return fixed&&fixed!==str?fixed:str;
  }catch(e){return String(s||'');}
}
function decodeHtmlEntities(s){
  const str=String(s||'');
  if(!str||!/[&][#a-z0-9]+;/i.test(str))return str;
...
function prettyId(s){
  const str=String(s||'');
  if(!str)return str;
  return decodeHtmlEntities(fixMojibake(str));
}
...
function adaptMatchedElement(el,type){
  if(!el)return el;
  const want=nodeSvgTag(type);
  if(!want)return el;
  const tag=el.tagName.toLowerCase();
  const ok = tag===want || (want==='path'&&'path polygon polyline'.includes(tag)) || (want==='ellipse'&&'circle ellipse'.includes(tag));
  if(ok)return el;
  if(tag==='g'){
    const found = el.querySelector(want==='path'?'path,polygon,polyline':want==='ellipse'?'ellipse,circle':want);
    if(found)return found;
  }
  return el;
}
function canRepresentType(el,type){
  if(!el)return false;
  const want=nodeSvgTag(type);
  if(!want)return true;
  const tag=el.tagName.toLowerCase();
  if(tag===want)return true;
  if(want==='path'&&'path polygon polyline'.includes(tag))return true;
  if(want==='ellipse'&&'circle ellipse'.includes(tag))return true;
  if(tag==='g')return !!el.querySelector(want==='path'?'path,polygon,polyline':want==='ellipse'?'ellipse,circle':want);
  return false;
}
function pickCandidate(candidates,used,type,parentSvg,strictParent=false){
  if(!candidates?.length)return null;
  let list=candidates.filter(el=>canRepresentType(el,type));
  if(parentSvg){
    const inside=list.filter(el=>el===parentSvg||parentSvg.contains(el));
    if(inside.length)list=inside;
    else if(strictParent)return null;
  }
  if(!list.length)return null;
  return list.find(el=>!used.has(el))||list[0];
}
function matchLayoutNodes(root){
  layoutToSvg.clear();
  if(!svg)return;

  const pool=[...svg.querySelectorAll('*')];
  const nodes=[];
  ...
  pool.forEach((el,idx)=>{
    ...
    const normKeys=[fid,id,dataName].filter(Boolean).map(normK).filter(Boolean);
    const tailKeys=[fid,id,dataName].filter(Boolean).map(tailK).filter(Boolean);
    const nameKeys=[dataName,id].filter(Boolean).map(normK).filter(Boolean);
    const node={idx,el,tag,id,fid,dataName};
    nodes.push(node);
  ...
  walkNodes(root,n=>{
    ...
    if(n.id||n.name||n.type)layoutNodes.push(n);
  });
  ...
  const firstFree=(list)=>{
    if(!list?.length)return null;
    return list.find(x=>!used.has(x.idx))||list[0]||null;
  };
  ...
  layoutNodes.forEach(n=>{
    ...
    if(n.id){ hit=firstFree(byExact.get(String(n.id))); if(hit)mode='exact'; }
    if(!hit&&n.id){ hit=firstFree(byNorm.get(normK(n.id))); if(hit)mode='norm-id'; }
    if(!hit&&n.id){ const tk=tailK(n.id); if(tk){ hit=firstFree(byTail.get(tk)); if(hit)mode='tail-id'; } }
    if(!hit&&n.name){ hit=firstFree(byName.get(normK(n.name))); if(hit)mode='name'; }
    if(!hit&&n.name){ const tk=tailK(n.name); if(tk){ hit=firstFree(byTail.get(tk)); if(hit)mode='tail-name'; } }

    if(!hit){
      const tag=nodeSvgTag(type);
      if(tag){
        for(let i=seq;i<nodes.length;i++){
          const c=nodes[i];
    ...
    if(hit){
      const el=adaptMatchedElement(hit.el,n.type);
      used.add(hit.idx);
      if(n.id)layoutToSvg.set(n.id,el);
      matched++;
      if(samples.length<20)samples.push({jsonId:n.id||'',jsonName:n.name||'',type:n.type||'',svgId:prettyId(hit.id||hit.fid||''),svgTag:hit.tag,mode});
    }else if(misses.length<20){
      misses.push({jsonId:n.id||'',jsonName:n.name||'',type:n.type||''});
    }
  });

  lgL('matching complete',{layoutNodes:layoutToSvg.size,svgNodes:nodes.length,matched});
  lgL('matching samples',samples);
  if(misses.length)lgL('matching misses',misses);
}
...
function resolveLayoutRoot(data){
  ...
  const all=[];
  walkNodes(data,n=>all.push(n));
  if(svgMap.size){
    const bySvg=all.find(n=>n?.id&&svgMap.has(n.id));
    if(bySvg)return bySvg;
  }
  const byLayout=all.find(n=>n&&(n.id||n.type||n.name)&&((n.layoutMode&&n.layoutMode!=='NONE')||(n.layout?.layoutMode&&n.layout.layoutMode!=='NONE')||(Array.isArray(n.children)&&n.children.length)||n.type==='FRAME'));
...
function wrapText(el,text,maxWidth){
  const host=el.tagName?.toLowerCase()==='tspan'?(el.closest('text')||el):el;
  const fs=parseFloat(host.getAttribute('font-size'))||16;
  const x=parseFloat(host.getAttribute('x'))||0;
  const y=parseFloat(host.getAttribute('y'))||fs;
  const lh=fs*1.2;
  if(!maxWidth||!isFinite(maxWidth)||maxWidth<=0){host.textContent=text;return;}
  const measure=(line)=>{
    const t=document.createElementNS('http://www.w3.org/2000/svg','tspan');
    t.textContent=line;host.appendChild(t);
  ...
  const splitLongToken=(token)=>{
    ...
    for(const ch of token){
      const test=cur+ch;
      if(cur&&measure(test)>maxWidth){parts.push(cur);cur=ch;}else cur=test;
    }
  ...
  text.split('\n').forEach(p=>{
    ...
    let line='';
    words.forEach(w=>{
      const chunks=measure(w)>maxWidth?splitLongToken(w):[w];
      chunks.forEach(chunk=>{
        const test=line?line+' '+chunk:chunk;
        const long=measure(test)>maxWidth;
        if(long&&line){lines.push(line);line=chunk;}else line=test;
      });
    });
    lines.push(line);
  });
  ...
  lines.forEach((ln,i)=>{
    const t=document.createElementNS('http://www.w3.org/2000/svg','tspan');
    t.setAttribute('x',x);
    t.setAttribute('y',y+i*lh);
    t.textContent=ln;
    host.appendChild(t);
  });
}

function getMappedLayoutNodeByElement(el){
  for(const [layoutId,svgEl] of layoutToSvg.entries()){
    if(svgEl===el)return LM.get(layoutId)||null;
  }
  return null;
}
function getElementMetrics(el){
  ...
  if((!b.width&&!b.height)&&el?.tagName?.toLowerCase()==='g'){
    const svgRoot=el.ownerSVGElement||svg;
    ...
    if(inv){
      ...
      if(isFinite(minX)&&isFinite(minY)&&isFinite(maxX)&&isFinite(maxY)){
        b={x:minX,y:minY,width:Math.max(0,maxX-minX),height:Math.max(0,maxY-minY)};
      }
    }
    if(!b.width&&!b.height){
      const tr=gTr(el);
      b.x=tr.x;b.y=tr.y;
      const ln=getMappedLayoutNodeByElement(el);
      if(ln){
        b.width=ln.size?.w||b.width;
        b.height=ln.size?.h||b.height;
        if(!b.x&&!b.y&&ln.pos){b.x=ln.pos.x||0;b.y=ln.pos.y||0;}
      }
    }
  }
  return b;
...
function measureNode(n){
  const el = layoutToSvg.get(n.id)||svgMap.get(n.id);

  if(n.type==='TEXT' && el){
    wrapText(el,n.text.content,n.text.maxWidth);
    const box = el.getBBox();
    n.size.w = n.text.maxWidth || box.width;
    n.size.h = box.height;
  }

  n.children.forEach(measureNode);

  if(n.layout.mode==='VERTICAL'){
    let h=0,w=0;
    n.children.forEach((c,i)=>{
      h+=c.size.h+(i?n.layout.gap:0);
      w=Math.max(w,c.size.w);
    });
    n.size.h = h + n.layout.padding.t + n.layout.padding.b;
    n.size.w = w + n.layout.padding.l + n.layout.padding.r;
  }

  if(n.layout.mode==='HORIZONTAL'){
    let w=0,h=0;
    n.children.forEach((c,i)=>{
      w+=c.size.w+(i?n.layout.gap:0);
      h=Math.max(h,c.size.h);
    });
    n.size.w = w + n.layout.padding.l + n.layout.padding.r;
    n.size.h = h + n.layout.padding.t + n.layout.padding.b;
  }
...
function layoutNode(n,x=0,y=0){
  n.pos.x=x;
  n.pos.y=y;

  if(n.layout.mode==='NONE'){
    n.children.forEach(c=>{
      const nBase=n.hasAbs?n.abs:n.orig;
      const cBase=c.hasAbs?c.abs:c.orig;
      const relX=(cBase?.x||0)-(nBase?.x||0);
      const relY=(cBase?.y||0)-(nBase?.y||0);
      layoutNode(c,x+relX,y+relY);
    });
    return;
  }

  let cx=x+n.layout.padding.l;
  let cy=y+n.layout.padding.t;

  n.children.forEach(c=>{
    layoutNode(c,cx,cy);

    if(n.layout.mode==='VERTICAL'){
      cy+=c.size.h+n.layout.gap;
    }else{
      cx+=c.size.w+n.layout.gap;
    }
  });
}

function renderNode(n,parentPos={x:0,y:0}){
  const el = layoutToSvg.get(n.id)||svgMap.get(n.id);
  if(!el) return;

  const lx=n.pos.x-parentPos.x;
  const ly=n.pos.y-parentPos.y;

  if(n.type==='TEXT'){
    const fs=parseFloat(el.getAttribute('font-size'))||16;
    el.setAttribute('x',lx);
    el.setAttribute('y',ly+fs);
    wrapText(el,n.text.content,n.text.maxWidth);

    el.onclick=()=>{
      const val = prompt('Edit text',n.text.content);
      if(val!==null){
        n.text.content = val;
        relayout();
      }
    ...
    const tag = el.tagName.toLowerCase();
    if(tag==='g'){
      // For nested groups, use local coordinates relative to parent to avoid cumulative drift.
      sTr(el,Math.round(lx*10)/10,Math.round(ly*10)/10);
    }else{
      const hasXY = el.hasAttribute('x')||el.hasAttribute('y');
      if(el.hasAttribute('x')) el.setAttribute('x',lx);
      if(el.hasAttribute('y')) el.setAttribute('y',ly);
      if(el.hasAttribute('width')) el.setAttribute('width',n.size.w);
      if(el.hasAttribute('height')) el.setAttribute('height',n.size.h);
      if(!hasXY) sTr(el,Math.round(lx*10)/10,Math.round(ly*10)/10);
    }
  }

  n.children.forEach(c=>renderNode(c,n.pos));
}

function captureLayoutBase(){
  if(!svg)return;
  const attrs=['x','y','width','height','cx','cy','r','rx','ry','x1','y1','x2','y2','transform'];
  svg.querySelectorAll('*').forEach(el=>{
    if(layoutBase.has(el))return;
    const snap={attrs:{},text:el.textContent||''};
    attrs.forEach(a=>{snap.attrs[a]=el.hasAttribute(a)?el.getAttribute(a):null;});
    layoutBase.set(el,snap);
  });
}
function restoreFromLayoutBase(el){
  const snap=layoutBase.get(el);
  if(!snap)return;
  Object.entries(snap.attrs).forEach(([a,v])=>{if(v===null)el.removeAttribute(a);else el.setAttribute(a,v);});
  if(el.tagName?.toLowerCase()==='text'||el.tagName?.toLowerCase()==='tspan')el.textContent=snap.text||'';
}
function resetLayoutTargets(){
  layoutToSvg.forEach(el=>restoreFromLayoutBase(el));
}
...
function relayout(){
  ...
  const base=rootNode.hasAbs?rootNode.abs:rootNode.orig;
  layoutNode(rootNode,base?.x||0,base?.y||0);
  renderNode(rootNode);
}
...
function openM(id){
  ...
  if(id==='aM'){rJ();rDL();}
  if(id==='ljM'){
    const fn=window.updLayoutLogUI||window.updateLayoutLogUI;
...
const layoutLogs=[];
function updLayoutLogUI(){const el=$('ljLOG');if(!el)return;el.textContent=layoutLogs.join('\n')||'Лог пуст';el.scrollTop=el.scrollHeight;}
function clearLayoutLogs(){layoutLogs.length=0;updLayoutLogUI();const m=$('ljMAP');if(m)m.innerHTML='';}
window.updLayoutLogUI=updLayoutLogUI;
function lgL(msg,data){
  const line='['+new Date().toLocaleTimeString()+'] '+msg+(data!==undefined?' '+JSON.stringify(data):'');
  layoutLogs.push(line);if(layoutLogs.length>200)layoutLogs.shift();
  updLayoutLogUI();
  if(data!==undefined)console.log('[LayoutImport]',msg,data);else console.log('[LayoutImport]',msg);
}
function applyLayoutPayload(data,src='unknown'){
  lgL('apply start: '+src,{svgLoaded:!!svg,svgMapped:svgMap.size});
  if(data?.svg){lgL('svg detected in payload, loading svg first');loadSVG(data.svg);}
  const payload=data?.layoutJson??data?.layout??data;
  loadLayoutJSON(payload,src);
}
async function handlePackFile(file){
  ...
  try{
    if(name.endsWith('.json')){
      ...
      applyLayoutPayload(data,'file:'+file.name);
      return;
    }
    if(!name.endsWith('.zip')){alert('Поддерживаются только .zip или .json');return;}
    if(typeof JSZip==='undefined'){alert('ZIP importer не загружен (JSZip)');return;}
    const zip=await JSZip.loadAsync(file);
    const names=Object.keys(zip.files);
    const svgName=names.find(n=>n.toLowerCase().endsWith('.svg'));
    const jsonName=names.find(n=>n.toLowerCase().includes('layout')&&n.toLowerCase().endsWith('.json'))||names.find(n=>n.toLowerCase().endsWith('.json'));
    if(!svgName){alert('В архиве не найден .svg файл');return;}
    const svgText=await zip.file(svgName).async('string');
    loadSVG(svgText);
    if(jsonName){
      const jsonText=await zip.file(jsonName).async('string');
      const data=JSON.parse(jsonText);
      applyLayoutPayload(data,'zip:'+file.name+'/'+jsonName);
    }
  }catch(err){
    console.error('Pack import error',err);
    alert('Ошибка импорта SVG+Layout: '+(err?.message||err));
  }
}
function applyLayoutJSONText(){
  const st=$('ljST');
  try{
    const txt=$('ljTA').value.trim();
    if(!txt){st.textContent='Пустой JSON';st.style.color='var(--red)';return;}
    const data=JSON.parse(txt);
    applyLayoutPayload(data,'manual-text');
    st.textContent='JSON применён (см. консоль [LayoutImport])';
    st.style.color='var(--grn)';
  }catch(err){
    st.textContent='Ошибка JSON: '+(err?.message||err);
    st.style.color='var(--red)';
  }
}
function analyzeJsonSvgIds(){
  const st=$('ljST');
  try{
    const txt=$('ljTA').value.trim();
    if(!txt){st.textContent='Вставьте JSON для анализа';st.style.color='var(--red)';return;}
    const data=JSON.parse(txt);
    const root=resolveLayoutRoot(data);
    if(!root){st.textContent='Не удалось найти root в JSON';st.style.color='var(--red)';return;}
    const jIds=[];
    walkNodes(root,n=>{if(n?.id)jIds.push(String(n.id));});
    mapSVG();
    const sIds=[...svgMap.keys()];
    const sNorm=new Set(sIds.map(normK));
    const sTail=new Set(sIds.map(tailK).filter(Boolean));
    const exact=jIds.filter(id=>svgMap.has(id));
    const norm=jIds.filter(id=>!svgMap.has(id)&&sNorm.has(normK(id)));
    const tail=jIds.filter(id=>!svgMap.has(id)&&!sNorm.has(normK(id))&&sTail.has(tailK(id)));
    const miss=jIds.filter(id=>!svgMap.has(id)&&!sNorm.has(normK(id))&&!sTail.has(tailK(id)));
    const rows=[];
    jIds.forEach(id=>{
      let mode='miss',svgId='—';
      if(svgMap.has(id)){mode='exact';svgId=id;}
      else{
        const n=normK(id),t=tailK(id);
        const nHit=sIds.find(x=>normK(x)===n);
        const tHit=sIds.find(x=>tailK(x)===t&&t);
        if(nHit){mode='norm';svgId=nHit;}
        else if(tHit){mode='tail';svgId=tHit;}
      }
      rows.push({jsonId:id,svgId,mode});
    });
    renderIdMapTable(rows);
    lgL('id-analyze',{jsonIds:jIds.length,svgIds:sIds.length,exact:exact.length,norm:norm.length,tail:tail.length,miss:miss.length});
    lgL('id-samples',{exact:exact.slice(0,15),norm:norm.slice(0,15),tail:tail.slice(0,15),miss:miss.slice(0,15)});
    st.textContent=`ID анализ: exact ${exact.length}, norm ${norm.length}, tail ${tail.length}, miss ${miss.length}`;
    st.style.color=miss.length?'var(--orn)':'var(--grn)';
  }catch(err){
    st.textContent='Ошибка анализа: '+(err?.message||err);
    st.style.color='var(--red)';
  }
}
function renderIdMapTable(rows){
  const box=$('ljMAP');if(!box)return;
  if(!rows?.length){box.innerHTML='<span style="color:var(--t3)">Таблица пуста</span>';return;}
  const color=m=>m==='exact'?'var(--grn)':m==='norm'?'var(--acc)':m==='tail'?'var(--orn)':'var(--red)';
  const top=rows.slice(0,250).map(r=>`<tr><td style="padding:4px 6px;border-bottom:1px solid var(--brd)">${esc(prettyId(r.jsonId))}</td><td style="padding:4px 6px;border-bottom:1px solid var(--brd)">${esc(prettyId(r.svgId))}</td><td style="padding:4px 6px;border-bottom:1px solid var(--brd);color:${color(r.mode)}">${r.mode}</td></tr>`).join('');
  box.innerHTML=`<div style="font-size:9px;color:var(--t3);margin-bottom:6px">Показано ${Math.min(rows.length,250)} из ${rows.length}</div><table style="width:100%;border-collapse:collapse;font-family:'JetBrains Mono',monospace"><thead><tr><th style="text-align:left;padding:4px 6px;border-bottom:1px solid var(--brd2)">JSON id</th><th style="text-align:left;padding:4px 6px;border-bottom:1px solid var(--brd2)">SVG id</th><th style="text-align:left;padding:4px 6px;border-bottom:1px solid var(--brd2)">Match</th></tr></thead><tbody>${top}</tbody></table>`;
}
...
function loadSVG(t){
  ...
  mapSVG();
  captureLayoutBase();
  s.removeEventListener('click',onSvgClick);
  s.addEventListener('click',onSvgClick);
  s.querySelectorAll('*').forEach(el=>{el.style.cursor='pointer';});
  $('dz').classList.add('has');$('hlo').style.display='none';sP(null);bT();detFonts();uDB();fitV();
}
function tagA(el){const id='n'+(nc++);el.dataset.nid=id;nM.set(id,el);for(const c of el.children)tagA(c);}

function loadLayoutJSON(json,src='unknown'){
  LM.clear();
  lgL('loadLayoutJSON start: '+src,{hasJson:!!json,svgMapped:svgMap.size});
  const root = resolveLayoutRoot(json);
  if(!root){console.warn('[LayoutImport] Layout root not found',json);return;}
  let matched=0,total=0;
  walkNodes(root,n=>{if(n?.id){total++;if(svgMap.has(n.id))matched++;}});
  lgL('resolved root',{id:root.id,type:root.type,children:root.children?.length||0,totalNodes:total,matchedSvgNodes:matched});
  if(!svgMap.size){lgL('abort: svgMap is empty (no data-figma-id or id attributes found)');return;}
  if(!total&&!(Array.isArray(root.children)&&root.children.length)){lgL('abort: resolved root has no traversable nodes',root);return;}
  matchLayoutNodes(root);
  rootNode = build(root);
  lgL('build complete',{lmSize:LM.size,rootId:rootNode?.id});
  relayout();
  lgL('relayout complete');
}
...
else if(eid)lt='#'+prettyId(eid);else if(cls)lt='.'+cls.split(/\s+/)[0];
else{const txt=el.textContent?.trim();if(('text tspan'.includes(tag))&&txt)lt='"'+txt.slice(0,18)+(txt.length>18?'…':'')+'"';}
lb.textContent=lt;
...
function mvL(sn,tn,z){const s=nM.get(sn),t=nM.get(tn);if(!s||!t||s.contains(t))return;const p=t.parentNode;if(z==='above')p.insertBefore(s,t);else if(z==='below')p.insertBefore(s,t.nextSibling);else{const tg=t.tagName.toLowerCase();'g svg defs clipPath mask symbol marker a pattern'.split(' ').includes(tg)?t.appendChild(s):p.insertBefore(s,t.nextSibling);}bT();selN(sn);}
function mvU(){if(!selEl||selEl===svg)return;const nx=selEl.nextElementSibling;if(nx){const n=selEl.dataset.nid;selEl.parentNode.insertBefore(nx,selEl);bT();selN(n);}}
function mvD(){if(!selEl||selEl===svg)return;const p=selEl.previousElementSibling;if(p){const n=selEl.dataset.nid;selEl.parentNode.insertBefore(selEl,p);bT();selN(n);}}
function delS(){if(!selEl||selEl===svg)return;const n=selEl.dataset.nid;selEl.remove();nM.delete(n);dynM.delete(n);selEl=null;$('hlo').style.display='none';bT();sP(null);uDB();}
...
function showMultiProps(){
  ...
  selSet.forEach(nid=>{
    ...
    const tag=el.tagName.toLowerCase();
    const name=el.id?'#'+prettyId(el.id):el.textContent?.trim().slice(0,20)||tag;
...
function groupSel(){
  ...
  const indexMap=new Map(Array.from(parent.children,(node,idx)=>[node,idx]));
  const sorted=[...els].sort((a,b)=>indexMap.get(a)-indexMap.get(b));
...
// Ungroup
function ungroupSel(){
  if(!selEl||selEl.tagName.toLowerCase()!=='g'||selEl===svg)return;
...
function gTr(el){
  ...
  const mt=t.match(/translate\(\s*([-\d.]+)(?:[,\s]+([-\d.]+))?\s*\)/);
  if(mt)return{x:+mt[1]||0,y:+(mt[2]??0)||0};
  const mm=t.match(/matrix\(\s*[-\d.eE]+[ ,]+[-\d.eE]+[ ,]+[-\d.eE]+[ ,]+[-\d.eE]+[ ,]+([-\d.eE]+)[ ,]+([-\d.eE]+)\s*\)/);
  if(mm)return{x:+mm[1]||0,y:+mm[2]||0};
  return{x:0,y:0};
}
...
if(el.id)h+=`<div class="prr"><span class="prk">ID</span><span class="prv">${esc(prettyId(el.id))}</span></div>`;
...
if(isR){
  ...
  h+=`<div class="prs"><div class="prst">Размер и позиция</div><div class="pig" id="pI" data-role="pos-grid">`;
...
h+=`<div class="pig" id="pI" data-role="pos-grid">`;if(pi._t){const t=gTr(el);let b={x:0,y:0,width:0,height:0};try{b=el.getBBox();}catch(e){}if((!b.width&&!b.height))b=getElementMetrics(el);h+=mI('X','tx',t.x)+mI('Y','ty',t.y)+`<div class="pip"><span class="pil">W</span><input class="pin" type="number" step="1" value="${Math.round(b.width*100)/100}" disabled></div><div class="pip"><span class="pil">H</span><input class="pin" type="number" step="1" value="${Math.round(b.height*100)/100}" disabled></div>`;}else for(const[l,a]of Object.entries(pi))h+=mI(l.toUpperCase(),a,parseFloat(el.getAttribute(a))||0);h+=`</div></div>`;}
h+=`<div class="pig" id="pI" data-role="pos-grid">`;if(pi._t){let b={x:0,y:0,width:0,height:0};try{b=el.getBBox();}catch(e){}if((!b.width&&!b.height))b=getElementMetrics(el);h+=mI('X','gx',b.x)+mI('Y','gy',b.y)+`<div class="pip"><span class="pil">W</span><input class="pin" type="number" step="1" value="${Math.round(b.width*100)/100}" disabled></div><div class="pip"><span class="pil">H</span><input class="pin" type="number" step="1" value="${Math.round(b.height*100)/100}" disabled></div>`;}else for(const[l,a]of Object.entries(pi))h+=mI(l.toUpperCase(),a,parseFloat(el.getAttribute(a))||0);h+=`</div></div>`;}
// Layout section (groups only)
if(tag==='g'&&!isR){
  const ly=LAY.get(nid)||{on:false,dir:'v',gap:0,pt:0,pr:0,pb:0,pl:0,align:'start'};
  ...
  // Direction
  h+=`<div class="lay-row"><span class="prk">Dir</span><div class="lay-dirs"><button class="lay-dir${ly.dir==='h'?' act':''}" data-ld="h" title="Horizontal"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="15 8 19 12 15 16"/></svg></button><button class="lay-dir${ly.dir==='v'?' act':''}" data-ld="v" title="Vertical"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"/><polyline points="8 15 12 19 16 15"/></svg></button></div></div>`;
  // Alignment
...
h+=`</div>`;}
if('text tspan'.includes(tag))h+=`<div class="prs"><div class="prst">Текст</div><div class="pcr"><span class="prk">Текст</span><input class="pin" style="width:150px" id="pTx" value="${esc(el.textContent||'')}"></div></div>`;
...
function pCR(a,v){const isNone=(v||'').trim()==='none';const hx=c2h(isNone?'#000':(v||'#000'));return `<div class="pcr"><span class="prk">${a}</span><label class="clrf" style="gap:4px"><input type="checkbox" data-ap-none="${a}" ${isNone?'checked':''} style="accent-color:var(--acc)"><span class="clrl" style="font-size:8px">none</span></label><input type="color" class="pci" data-ap="${a}" value="${hx}" ${isNone?'disabled':''}><input class="pin" style="width:68px" data-ap="${a}" value="${esc(v)}" placeholder="none" ${isNone?'disabled':''}></div>`;}
...
function c2h(c){if(!c||c==='none'||c==='transparent')return'#000000';if(/^#[0-9a-f]{6}$/i.test(c))return c;if(/^#[0-9a-f]{3}$/i.test(c))return'#'+c[1]+c[1]+c[2]+c[2]+c[3]+c[3];if(c.startsWith('rgb')){const m=c.match(/(\d+)/g);if(m?.length>=3)return'#'+m.slice(0,3).map(n=>(+n).toString(16).padStart(2,'0')).join('');}try{if(!colorCanvasCtx)return'#000000';colorCanvasCtx.fillStyle=c;return colorCanvasCtx.fillStyle;}catch(e){}return'#000000';}
...
function applyLayout(gEl){
  ...
  for(const ch of gEl.children){
    if(ch.dataset?.nid&&!hidS.has(ch.dataset.nid)){
      try{const b=ch.getBBox();if(b.width||b.height)kids.push({el:ch,b});}catch(e){}
    }
  }
  if(!kids.length)return;
  const isH=ly.dir==='h';
  const gBbox=gEl.getBBox();
  // Group's own translate
  const gTrV=gTr(gEl);
  let cursor=isH?ly.pl:ly.pt;
  // Cross-axis size for alignment
  let maxCross=0;
  kids.forEach(k=>{maxCross=Math.max(maxCross,isH?k.b.height:k.b.width);});
  const totalCross=maxCross+(isH?(ly.pt+ly.pb):(ly.pl+ly.pr));

  kids.forEach((k,i)=>{
    const el=k.el,b=k.b;
    // Calculate desired position relative to group
    let tx,ty;
    if(isH){
      tx=cursor;
      if(ly.align==='start')ty=ly.pt;
      else if(ly.align==='center')ty=ly.pt+(maxCross-b.height)/2;
      else ty=ly.pt+maxCross-b.height;
      cursor+=b.width+(i<kids.length-1?ly.gap:0);
    }else{
      ty=cursor;
      if(ly.align==='start')tx=ly.pl;
      else if(ly.align==='center')tx=ly.pl+(maxCross-b.width)/2;
      else tx=ly.pl+maxCross-b.width;
      cursor+=b.height+(i<kids.length-1?ly.gap:0);
    }
    // Apply offset: move child relative to its current bbox position
    const dx=tx-b.x+gBbox.x, dy=ty-b.y+gBbox.y;
    // Use transform translate for universal positioning
    const pi=pA(el);
    if(pi&&pi._t){
      const ct=gTr(el);
      sTr(el,Math.round((ct.x+dx)*10)/10,Math.round((ct.y+dy)*10)/10);
    }else if(pi&&pi.x1){
      el.setAttribute('x1',Math.round((+el.getAttribute('x1')+dx)*10)/10);
      el.setAttribute('y1',Math.round((+el.getAttribute('y1')+dy)*10)/10);
      el.setAttribute('x2',Math.round((+el.getAttribute('x2')+dx)*10)/10);
      el.setAttribute('y2',Math.round((+el.getAttribute('y2')+dy)*10)/10);
    }else if(pi){
      const xa=pi.x||pi.cx||'x',ya=pi.y||pi.cy||'y';
      el.setAttribute(xa,Math.round(((+el.getAttribute(xa)||0)+dx)*10)/10);
      el.setAttribute(ya,Math.round(((+el.getAttribute(ya)||0)+dy)*10)/10);
    }else{
      const ct=gTr(el);sTr(el,Math.round((ct.x+dx)*10)/10,Math.round((ct.y+dy)*10)/10);
    }
  });
  hl(selEl);
}
...
if(isR){
  const pw=$('pI');
  if(pw)pw.querySelectorAll('.pin').forEach(i=>i.addEventListener('input',()=>{
...
pw.querySelectorAll('.pin').forEach(i=>i.addEventListener('input',()=>{
  const a=i.dataset.pa,v=+i.value||0;
  if(tf){
    if(a!=='tx'&&a!=='ty')return;
    const nVal=Number(i.value);
    if(!Number.isFinite(nVal))return;
    const t=gTr(el);
    if(a==='tx')sTr(el,Math.round(nVal*10)/10,t.y);
    else sTr(el,t.x,Math.round(nVal*10)/10);
    const nt=gTr(el);
    const xI=pw.querySelector('[data-pa="tx"]'),yI=pw.querySelector('[data-pa="ty"]');
    if(xI)xI.value=Math.round(nt.x*10)/10;
    if(yI)yI.value=Math.round(nt.y*10)/10;
    let b={x:0,y:0,width:0,height:0};try{b=el.getBBox();}catch(e){}if((!b.width&&!b.height))b=getElementMetrics(el);const t=gTr(el);
    if(a==='gx')sTr(el,Math.round((t.x+(nVal-b.x))*10)/10,t.y);
    else sTr(el,t.x,Math.round((t.y+(nVal-b.y))*10)/10);
    let nb={x:0,y:0,width:0,height:0};try{nb=el.getBBox();}catch(e){}if((!nb.width&&!nb.height))nb=getElementMetrics(el);
    hl(el);
    return;
  }
  else{
...
pw.querySelectorAll('.pil').forEach(lb=>lb.addEventListener('mousedown',e=>{e.preventDefault();const a=lb.dataset.sc,inp=pw.querySelector(`[data-pa="${a}"]`);let sx=e.clientX,sv=+inp.value||0;const mv=e2=>{inp.value=Math.round((sv+e2.clientX-sx)*100)/100;inp.dispatchEvent(new Event('input'));},up=()=>{removeEventListener('mousemove',mv);removeEventListener('mouseup',up);document.body.style.cursor='';};addEventListener('mousemove',mv);addEventListener('mouseup',up);document.body.style.cursor='ew-resize';}));}
document.querySelectorAll('[data-ap]').forEach(i=>{const h=()=>{const a=i.dataset.ap,v=i.value;el.setAttribute(a,v);hl(el);const r=i.closest('.pcr');if(!r)return;if(i.type==='color'){const t=r.querySelector('.pin[data-ap="'+a+'"]');if(t)t.value=v;}else{const c=r.querySelector('.pci[data-ap="'+a+'"]');if(c&&v&&v!=='none')try{c.value=c2h(v);}catch(e){}}};i.addEventListener('input',h);i.addEventListener('change',h);});
document.querySelectorAll('[data-ap-none]').forEach(cb=>cb.addEventListener('change',()=>{const a=cb.dataset.apNone;const r=cb.closest('.pcr');if(!r)return;const color=r.querySelector('.pci[data-ap=\"'+a+'\"]');const txt=r.querySelector('.pin[data-ap=\"'+a+'\"]');if(cb.checked){el.setAttribute(a,'none');if(txt)txt.value='none';if(color)color.disabled=true;if(txt)txt.disabled=true;}else{const val=(color?.value||'#000000');el.setAttribute(a,val);if(txt)txt.value=val;if(color)color.disabled=false;if(txt)txt.disabled=false;}hl(el);}));
const tx=$('pTx');if(tx)tx.addEventListener('input',()=>{
  // Preserve tspan structure — update first tspan or direct text node
  const tspans=el.querySelectorAll('tspan');
  if(tspans.length>0){tspans[0].textContent=tx.value;}
  else{
    // Find first text node or set directly
    let found=false;
    for(const n of el.childNodes){if(n.nodeType===3&&n.textContent.trim()){n.textContent=tx.value;found=true;break;}}
    if(!found)el.textContent=tx.value;
  }
  hl(el);
});
...
if(layOn){
  function getLay(){return LAY.get(nid)||{on:false,dir:'v',gap:0,pt:0,pr:0,pb:0,pl:0,align:'start'};}
  function setLay(upd){const ly={...getLay(),...upd};LAY.set(nid,ly);if(ly.on)applyLayout(el);}
  layOn.addEventListener('change',()=>{
    if(layOn.checked){if(!LAY.has(nid))LAY.set(nid,{on:true,dir:'v',gap:8,pt:0,pr:0,pb:0,pl:0,align:'start'});else{const l=getLay();l.on=true;LAY.set(nid,l);}if(layDet)layDet.style.display='block';applyLayout(el);}
...
vp.addEventListener('mousedown',e=>{
  ...
  const hit=e.target.closest('[data-nid]');
  if(hit&&hit!==svg){isDrEl=true;dES={x:e.clientX,y:e.clientY};e.preventDefault();const pi=pA(hit);
...
document.addEventListener('keydown',e=>{if(e.target.tagName==='INPUT'||e.target.tagName==='TEXTAREA')return;if(e.key==='v'||e.key==='Escape'){setTool('select');selSet.clear();if(selEl)selN(selEl.dataset.nid);}if(e.key==='r'&&!e.ctrlKey)setTool('rect');if(e.key==='c'&&!e.ctrlKey)setTool('circle');if(e.key==='t'&&!e.ctrlKey)setTool('text');if((e.key==='Delete'||e.key==='Backspace')&&selEl&&selEl!==svg&&e.target===document.body){e.preventDefault();delS();}if(e.key==='g'&&(e.ctrlKey||e.metaKey)&&!e.shiftKey){e.preventDefault();groupSel();}if(e.key==='g'&&(e.ctrlKey||e.metaKey)&&e.shiftKey){e.preventDefault();ungroupSel();}});
function gC(){return{fi:$('tNF').checked?'none':$('tFi').value,st:$('tNS').checked?'none':$('tSt').value,sw:+$('tSW').value||0};}
$('tFi').addEventListener('input',e=>{$('tFiV').style.background=e.target.value;applyToolbarToSelection();});
$('tSt').addEventListener('input',e=>{$('tStV').style.background=e.target.value;applyToolbarToSelection();});
function syncToolbarNoneUI(){const nf=$('tNF').checked,ns=$('tNS').checked;$('tFi').disabled=nf;$('tFiV').style.opacity=nf?'.45':'1';$('tSt').disabled=ns;$('tStV').style.opacity=ns?'.45':'1';$('tSW').disabled=ns;}
function applyToolbarToSelection(){if(!selEl||selEl===svg)return;const cl=gC();selEl.setAttribute('fill',cl.fi);if(cl.st==='none'){selEl.setAttribute('stroke','none');selEl.setAttribute('stroke-width',0);}else{selEl.setAttribute('stroke',cl.st);selEl.setAttribute('stroke-width',cl.sw);}hl(selEl);sP(selEl);}
$('tNF').addEventListener('change',()=>{syncToolbarNoneUI();applyToolbarToSelection();});
$('tNS').addEventListener('change',()=>{syncToolbarNoneUI();applyToolbarToSelection();});
$('tSW').addEventListener('input',applyToolbarToSelection);
syncToolbarNoneUI();
...
if(tool==='text'){const txt=prompt('Текст:','Text');if(!txt)return;const cl=gC(),el=document.createElementNS('http://www.w3.org/2000/svg','text');el.setAttribute('x',Math.round(pt.x));el.setAttribute('y',Math.round(pt.y));el.setAttribute('font-size','20');el.setAttribute('font-family','sans-serif');el.setAttribute('fill',cl.fi);if(cl.st!=='none'){el.setAttribute('stroke',cl.st);el.setAttribute('stroke-width',cl.sw);}el.textContent=txt;svg.appendChild(el);finN(el);setTool('select');return;}
isDraw=true;dSt=pt;const cl=gC();
...
function finN(el){tagA(el);el.addEventListener('click',e=>{e.stopPropagation();selN(el.dataset.nid,e);});el.style.cursor='pointer';bT();selN(el.dataset.nid);}
  ...
  if (tplId) {
    try {
      ...
      if (res.ok) {
        ...
        if (tpl.layoutJson) loadLayoutJSON(tpl.layoutJson);
